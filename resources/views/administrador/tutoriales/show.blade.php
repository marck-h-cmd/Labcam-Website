...
@section('contenido')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-3">
            <a href="{{ url()->previous() }}"
                class="inline-flex items-center justify-center p-2 rounded-lg  border-indigo-400 bg-white/80 backdrop-blur-sm shadow-sm hover:bg-indigo-50 ease-out transition-all"
                title="Volver atrás">
                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="#09244B">
                    <path
        ...
        <!-- Tutorial Titulo -->
        <h1 class="text-3xl font-bold text-gray-800 mb-5 block max-md:text-2xl">{{ $tutorial->titulo }}</h1>

        <!-- Videos Section -->
        @if (count($filesByType['videos']) > 0)
            <section class="mb-12">
                <div class="bg-blue-500 text-white px-6 py-3 rounded-t-lg flex justify-between items-center cursor-pointer"
                    onclick="toggleSection('videos-section')">
                    ...
                    <svg id="videos-arrow" class="w-5 h-5 transform transition-transform duration-200" viewBox="0 0 24 24"
                ...
                <div id="videos-section" class="bg-white p-6 rounded-b-lg shadow-md">
                    <div
                        class="@if (count($filesByType['videos']) === 1) w-full @else grid grid-cols-1 md:grid-cols-2 gap-6 @endif">
                        @foreach ($filesByType['videos'] as $video)
                            <div class="@if (count($filesByType['videos']) === 1) w-full @endif border rounded-lg overflow-hidden">
                                <div class="p-4">
        ...
        @if (count($filesByType['pdfs']) > 0)
            <section class="mb-12">
                <div class="bg-red-500 text-white px-6 py-3 rounded-t-lg flex justify-between items-center cursor-pointer"
                    onclick="toggleSection('pdfs-section')">
                    <h2 class="text-xl font-semibold flex items-center">
                    ...
                    <svg id="pdfs-arrow" class="w-5 h-5 transform transition-transform duration-200" viewBox="0 0 24 24"
                        fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7 10L12 15L17 10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                            stroke-linejoin="round"></path>
                    </svg>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        @foreach ($filesByType['pdfs'] as $pdf)
                            <a href="{{ $pdf['url'] }}" target="_blank" class="group">
                                <div
                                    class="border rounded-lg p-4 h-full flex flex-col items-center justify-center hover:bg-gray-50 transition-colors">
                                    ...
                                    <p
                                        class="text-sm font-medium text-gray-700 text-center group-hover:text-red-600 transition-colors truncate w-full">
                                        {{ $pdf['name'] }}</p>
                                </div>
                            </a>
                        @endforeach
                        ...
                        @foreach ($filesByType['archives'] as $archive)
                            <a href="{{ $archive['url'] }}" download class="group">
                                <div
                                    ...
                                    <p
                                        class="text-sm font-medium text-gray-700 text-center group-hover:text-yellow-600 transition-colors truncate w-full">
                                        {{ $archive['name'] }}</p>
                                </div>
                            </a>
                        @endforeach
