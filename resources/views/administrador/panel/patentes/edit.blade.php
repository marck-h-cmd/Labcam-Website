    ...
    <body>
        <div class="paper-container">
            <div class="main-title flex flex-col items-center gap-3 mb-8">
                <div class="title text-2xl font-semibold text-[#2e5382]">Editar Patente</div>
                <div class="blue-line w-1/5 h-0.5 bg-[#64d423]"></div>
            </div>

            <form class="p-10" action="{{ route('patentes.update', $patente->id) }}" method="post" id="form"
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <div>
                          ...
                          focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Titulo del patente" required value="{{ old('titulo', $patente->titulo) }}" />
                        </div>
                        ...
                        <div>
                            ...
                            focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                ...
                                <input type="hidden" name="autores" id="autores-json"
                                    value="{{ old('autores', $patente->autores) }}" />
                            </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha_publicacion" id="fecha_publicacion"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                          focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                required value="{{ old('fecha_publicacion', $patente->fecha_publicacion) }}" />
                        </div>
              ...
              {{ old('descripcion', $patente->descripcion) }}
            </textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="doi" id="doi"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                          focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="DOI" required value="{{ old('doi', $patente->doi) }}" />
                        </div>
                        ...
                        <div>
                           ...
                           focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                required>
                                <option value="" @if (old('area_id', $patente->area_id) === '') selected @endif>
                                    Selecciona un Área de Investigación
                                </option>
                                @if ($areas && !$areas->isEmpty())
                                    @foreach ($areas as $area)
                                        <option value="{{ $area->id }}"
                                            @if (old('area_id', $patente->area_id) == $area->id) selected @endif>
                                            {{ $area->nombre }}
                                        </option>
                                    @endforeach
                                @else
                        ...
                        <div>
                            ...
                            <div id="image-upload"
                                ...
                                <span id="image-placeholder"
                                    class="text-gray-500 flex flex-col items-center {{ $patente->img_filename ? 'hidden' : '' }}">
                                ...
                                <img id="previewImage"
                                    src="{{ $patente->img_filename ? Storage::url('uploads/paper_img/' . $patente->img_filename) : '' }}"
                                    alt="Vista previa"
                                    class="w-full h-full object-cover rounded shadow mx-auto {{ $patente->img_filename ? '' : 'hidden' }}">

                                <!-- Botón eliminar imagen (solo visible si hay imagen) -->
                                <button type="button" id="remove-image"
                                    class="{{ $patente->img_filename ? 'block' : 'hidden' }} absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full hover:bg-red-600 transition cursor-pointer"
                        ...
                        <div>
                            ...
                            <div id="pdf_filename-upload"
                                ...
                                <span id="pdf_filename-placeholder"
                                    class="text-gray-500 {{ $patente->pdf_filename ? 'hidden' : '' }}">
                                    Selecciona o arrastra un archivo PDF
                                </span>

                                <!-- Nombre de archivo PDF (se muestra si SÍ hay PDF) -->
                                <div id="pdf_filename-file-info"
                                    class="text-sm {{ $patente->pdf_filename ? '' : 'hidden' }}">
                                    {{ $patente->pdf_filename ?? '' }}
                                </div>

                                <!-- Botón eliminar PDF (solo visible si hay PDF) -->
                                <button type="button" id="remove-pdf_filename"
                                    class="{{ $patente->pdf_filename ? 'block' : 'hidden' }} absolute top-1 right-2 bg-red-500 text-white p-1 rounded-full hover:bg-red-600 transition cursor-pointer"
    ...
    @section('script')
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                @if ($patente->autores)
                    const precargados = JSON.parse(@json($patente->autores));
