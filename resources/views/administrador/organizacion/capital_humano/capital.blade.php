...
<div class="p-2">
    <div class="max-w-[1200px] mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <div class="w-full">
                <form class="sm:flex sm:items-center sm:justify-between flex-col sm:flex-row gap-4" action="{{route('direccion.store')}}" method="POST" id="form1">
                    @csrf
                        ...
                        <div class="inline-block min-w-full py-2 align-middle">
                            <table class="w-full divide-y divide-gray-300">
                                ...
                                <tbody id="table-body" class="divide-y divide-gray-200 bg-white">
                                    @foreach ($direccion)
                                    <tr data-role="{{ $direccion->rol }}" class="even:bg-gray-50">
                                        ...
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900">
                                            <form action="{{ route('direccion.destroy', $direccion->id) }}" method="POST">
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <a href="{{ route('direccion.destroy', $direccion->id) }}" class="text-red-600 font-bold hover:text-red-900" onclick="event.preventDefault(); 
                                                    Swal.fire({
                                                        title: '¿Estás seguro?',
                            ...
                            <div class="mt-4 px-4">
                                {!! $direccion->withQueryString()->links() !!}
                            </div>
                        </div>
                    </div>
                </div>
